...
export function ShareDialog({
  ...
  const shareUrl = shareCode
    ? `${window.location.origin}/collections?import=${shareCode}`
    : "";
        ...
        {!showSuccess ? (
          <div className="flex flex-col gap-5">
            <p className="text-sm leading-relaxed text-muted-foreground">
              Compartilhe "{objective?.name}" com outras pessoas. Elas poderão importar para sua própria biblioteca.
            </p>
          ...
          <div className="flex flex-col gap-5">
            ...
            <p className="text-center text-xs text-muted-foreground">
              Compartilhe o código "<span className="font-mono font-medium">{shareCode}</span>" ou o link acima.
            </p>
...
export function ImportDialog({
  ...
  const [loading, setLoading] = useState(false);
  const [preview, setPreview] = useState<import("@/lib/supabaseClient").SharedObjectiveWithItems | null>(null);
  ...
  const handleImport = async (copyToLibrary: boolean) => {
    ...
    if (result.error) {
      ...
      toast(copyToLibrary ? "Importado para sua biblioteca!" : "Adicionado às bibliotecas compartilhadas");
      onOpenChange(false);
