...
export function MultiplayerGame({
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      ...
      if (left <= 0) {
        clearInterval(t);
        onAnswer(step, current.itemId, "", false, secondsPerQuestion * 1000, secondsPerQuestion);
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        ...
        >
          <span
            className={
              "h-1.5 w-1.5 rounded-full " + (remaining <= 3 ? "bg-destructive" : "bg-primary")
            }
          />
      ...
      <QuestionCard
        item={memoItem}
        prompt={getPrompt(current.gameType)}
        step={step + 1}
        total={total}
      >
              ...
              {current.options.map((opt, idx) => {
                ...
                return (
                  ...
                  >
                    <Button
                      variant="outline"
                      onClick={() => confirmChoice(opt.id)}
                      disabled={!!picked}
                      className={
                        "press h-14 w-full justify-start rounded-2xl border-2 text-base font-semibold " +
                        state
                      }
                    >
                      {opt.label}
        ...
        {current.gameType === "order-words" && (
          <>
            ...
            <div
              className={
                "mt-4 min-h-24 rounded-2xl border-2 border-dashed p-3 " +
                (locked
                  ? chosen.every((w, i) => w.word === current.wordOrder![i])
                    ? "border-success bg-success/10"
            ...
            <Button
              ...
              disabled={chosen.length !== current.wordOrder?.length || locked}
...
function getPrompt(gameType: string): string {
  switch (gameType) {
    case "multiple-choice":
      return "A qual referência este texto pertence?";
    case "fill-blank":
      return "Complete a palavra que falta";
    case "order-words":
      return "Coloque as palavras na ordem correta";
    default:
      return "";
  }
}
...
function FillBlankContent({
  ...
  const blankIdx =
    eligible.length > 0
      ? eligible[Math.floor(eligible.length * 0.3)].i
      : Math.floor(tokens.length / 2);
  const answer = tokens[blankIdx]?.replace(/[.,;!?]/g, "") ?? "";
  const isRight =
    picked != null &&
    current.options.find((o) => o.id === picked)?.label.toLowerCase() === answer.toLowerCase();

  return (
    <>
      <motion.p
        animate={picked ? (isRight ? { scale: [1, 1.02, 1] } : { x: [0, -8, 8, -6, 6, 0] }) : {}}
      ...
      <div className="mt-5 grid grid-cols-2 gap-3">
        {current.options.map((opt, idx) => {
          const state =
            picked == null
              ? ""
              : opt.id === picked && opt.label.toLowerCase() === answer.toLowerCase()
                ? "border-success bg-success/15"
                : opt.id === picked
                  ? "border-destructive bg-destructive/15"
