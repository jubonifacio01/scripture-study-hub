...
if (!supabaseUrl || !supabaseAnonKey) {
  // Fail loudly instead of silently falling back to a project ID baked into the
  // bundle. A hardcoded fallback here means every environment (including forks
  // of this repo) that forgets to set env vars quietly talks to the same
  // Supabase project, which is surprising and hard to debug.
  throw new Error(
    "Supabase não configurado: defina VITE_SUPABASE_URL e VITE_SUPABASE_ANON_KEY no seu .env.",
  );
}
...
export async function shareObjective(
  ...
  if (!supabase) {
    return {
      shareCode: "",
      error: new Error("Supabase não configurado. Compartilhamento requer backend."),
    };
  }
  try {
    ...
    if (objectiveError || !objectiveData) {
      return {
        shareCode: "",
        error: objectiveError || new Error("Failed to create shared objective"),
      };
    }
