...
export async function loadProgressStats(): Promise<ProgressStats> {
  ...
  if (!progressRows || progressRows.length === 0) {
    ...
    const { data: objRows } = await supabase.from("objectives").select("id").eq("owner_id", userId);
  ...
  const totalAnswers = progressRows.reduce(
    (s: number, r: { total_answers: number }) => s + r.total_answers,
    0,
  );
  const correctAnswers = progressRows.reduce(
    (s: number, r: { correct_answers: number }) => s + r.correct_answers,
    0,
  );
