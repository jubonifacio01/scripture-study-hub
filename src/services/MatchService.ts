...
function difficultyToGameType(difficulty: MultiplayerDifficulty): GameType {
  switch (difficulty) {
    case "facil": return "multiple-choice";
    case "medio": return "fill-blank";
    case "dificil": return "order-words";
    case "aleatorio": {
...
export function buildQuestions(
  ...
  return chosen.map((it, idx) => {
    ...
    if (gameType === "fill-blank") {
      ...
      const blankIdx = eligible.length > 0
