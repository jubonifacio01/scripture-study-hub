...
export async function updateMemoryText(
  ...
  if (
    ...
    updates.book !== undefined ||
    updates.chapter !== undefined ||
    updates.verse !== undefined
  ) {
...
export async function deleteMemoryText(textId: string): Promise<ServiceResult<void>> {
  ...
  const { error } = await supabase.from("memory_texts").delete().eq("id", textId);
...
export async function duplicateObjectiveInDb(
  ...
  if (srcTexts && srcTexts.length > 0) {
    ...
    newTexts = ((inserted ?? []) as DbMemoryText[]).sort((a, b) => a.order_index - b.order_index);
  }
