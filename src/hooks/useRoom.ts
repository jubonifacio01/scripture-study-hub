...
export function useRoom({ code, isHost, enabled }: UseRoomArgs) {
  ...
  const startMatch = useCallback(
    async (cfg: RoomConfig, items: MemoryItem[], bank: MemoryItem[]) => {
      if (!roomRef.current) return;
      ...
      const questions = buildQuestions(
        items,
        bank,
        Math.min(cfg.questionCount, items.length),
        cfg.difficulty,
      );
      ...
      broadcast({
        ...
        payload: {
          questions,
          startAt,
          config: { ...cfg, questionCount: questions.length },
  ...
  const finishPlaying = useCallback(
    async (totalCorrect: number, totalQuestions: number, totalTimeMs: number) => {
      ...
      await updatePlayerScore(
        ...
        totalCorrect,
        totalTimeMs,
        Math.max(me.combo, comboRef.current),
        true,
      );

      await saveMatchProgress(
        match.config.objectiveId,
        totalCorrect,
        totalQuestions,
        xpEarned,
        totalTimeMs,
