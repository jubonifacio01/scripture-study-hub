...
function PlayPage() {
  ...
  const handleOpenChapter = (chapter: JourneyChapter) => {
    ...
    const targetSession = last && last.chapter.id === chapter.id ? last.session : firstSession;
  ...
  const handleContinueJourney = () => {
    ...
    const sessionIndex = activeChapter.sessions.findIndex((s) => s.id === activeSession?.id);
    ...
    const chapterIndex = journey.chapters.findIndex((c) => c.id === activeChapter.id);
            ...
            {journey.chapters.map((ch, i) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-[15px] font-semibold tracking-tight">{ch.title}</p>
