...
import { recordRoomHosted, recordMultiplayerResult } from "@/lib/multiplayerStats";
import { toast } from "sonner";
import type { MultiplayerDifficulty, RoomConfig, Objective, MemoryItem } from "@/types";
import {
  Users,
  Copy,
  Share2,
...
function RoomView({
  ...
  const start = () => {
    ...
    const bank = allItems.length > 0 ? allItems : items;
        ...
        <div className="flex flex-col gap-2">
          {room.players.length === 0 ? (
            <EmptyState
              ...
              description="Compartilhe o código para começar."
            />
          ) : (
      ...
      <section className="mt-6 card-elevated p-5">
        ...
        {isHost ? (
          <>
            ...
            {objectivesLoading ? (
              <p className="mt-1.5 text-sm text-muted-foreground">Carregando objetivos…</p>
            ) : objectives.length === 0 ? (
              <p className="mt-1.5 text-sm text-muted-foreground">
                Você ainda não tem objetivos. Crie um na tela de coleções antes de iniciar uma
                partida.
              </p>
            ) : (
              <select
          ...
          <>
            <div className="mt-4 space-y-3 text-sm">
              ...
              <Row
                label="Dificuldade"
                value={
                  displayDifficulty === "facil"
                    ? "Fácil"
                    : displayDifficulty === "medio"
                      ? "Médio"
                      : displayDifficulty === "dificil"
                        ? "Difícil"
                        : "Aleatório"
                }
              />
                  ...
                  >
                    ...
                    <span
                      className={
                        ...
                        (p.done ? "text-success" : "text-muted-foreground")
                      }
                    >
...
function ResultsView({
  ...
  useEffect(() => {
    ...
    if (ranking.length === 0 || !room.myPlayerId) return;
    const me = ranking.find((r) => r.playerId === room.myPlayerId);
    if (!me) return;
    recordedRef.current = true;
    const won = ranking.length > 1 && ranking[0]?.playerId === room.myPlayerId;
    recordMultiplayerResult(won, me.combo);
  }, [ranking, room.myPlayerId]);
        ...
        {ranking.map((p, i) => {
          ...
          return (
            <motion.div
              ...
              className={
                "card-elevated flex items-center gap-3 p-3 " + (isTop3 ? "ring-1 ring-border" : "")
              }
            >
              <div
                className={
                  "grid h-9 w-9 shrink-0 place-items-center rounded-full text-sm font-semibold tabular-nums " +
                  (isTop3 ? medal : "bg-muted text-muted-foreground")
                }
              >
                {i + 1}
              </div>
